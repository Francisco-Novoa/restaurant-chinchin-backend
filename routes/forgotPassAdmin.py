...
route_forgetpassadmin = Blueprint('route_forgetpassadmin', __name__)
...
@route_forgetpassadmin.route('/adminchange-password', methods=['POST'])
def change_password():
        ...
        confirm_url = 'http://localhost:3000/confirmationadmin/' + token
        html = render_template('email_confirmation.html', confirm_url=confirm_url)
        subject = "Por favor, Confirmar su email."
        sendMail("Por favor, Confirmar su email.", admin.email, html)
        return jsonify({"success": "Email send successfully"}), 200


@route_forgetpassadmin.route('/adminchange-password-confirm/<token>', methods=['POST'])
